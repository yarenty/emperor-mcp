...
impl ForwardedHeaders {
    ...
    pub fn from_request_context<R>(ctx: &rmcp::service::RequestContext<R>) -> Self
    where
...
impl ForwardConfig {
    ...
    {
        Self {
            allow: Arc::new(names.into_iter().map(Into::into).collect()),
        }
    }
...
mod tests {
    ...
    fn captures_only_allowlisted_headers() {
        ...
        let fwd =
            ForwardedHeaders::from_header_map(&headers, &["Authorization", "X-Upstream-User"]);
